...
export default async function confluenceNotesSyncHandler(
  ...
  if (team === undefined) {
    respondToWebsocket(client, event, {
      type: MessageType.CONFLUENCE_NOTES_SYNCED,
      response: "Unknown team to sync notes from",
    });
    return { statusCode: 200, body: "handled" };
  }

  const pageUrl = await syncConfluenceNotes(client, persistentId, team);
  respondToWebsocket(client, event, {
    type: MessageType.CONFLUENCE_NOTES_SYNCED,
...
async function syncConfluenceNotes(
  ...
  const actionItems = [
    {
      text: "Add support for previous action items",
      complete: false,
      user: "ggeorgalis",
    },
  ...
  let p = await findEndpointRetroPage(now, config, team);
  if (!p) {
    // If page does not exist, we should create a new confluence page
    let parent = await findEndpointRetroPagesParent(now, config, team);
...
function generateQuery(date: Date, team: Team, isParentPage: boolean): string {
  let query: string = `/rest/api/content?title=${formatRetroDate(date)}%20`;

  switch (team) {
    case Team.AMBER:
      query += "-%20Amber%20List";
      break;
    case Team.CLIENT:
      query += `Retrospective%20-%20Team++`;
      break;
    case Team.RIVIA:
      query += "Rivia%20Retrospective";
      break;
    default:
      console.error(`Failed to correctly idenity the team: ${team}`);
      return "";
  }

  let spaceKey: string = "EB";
  if (team === Team.CLIENT) {
    let spaceKey = "EAC";
  }
  ...
  if (isParentPage) {
    query += "&expand=children.page";
  } else {
    query += "&expand=page,version";
  }

  return query;
}

async function findEndpointRetroPage(
  date: Date,
  config: Config,
  team: Team
): Promise<ConfluencePage | undefined> {
  const findEndpointRetroPageQuery = generateQuery(date, team, false);
...
async function findEndpointRetroPagesParent(
  date: Date,
  config: Config,
  team: Team
): Promise<ConfluencePage | undefined> {
  const findEndpointRetroPagesParentQuery = generateQuery(date, team, true);
  const rawResponse = await axios.get(
    findEndpointRetroPagesParentQuery,
    config
  );
  const response = rawResponse.data;
...
async function createRetroPage(
  date: Date,
...
async function updateRetroPage(
  page: ConfluencePage,
  content: string,
  config: Config
): Promise<ConfluencePage | undefined> {
...
function formatRetroDate(date: Date): string {
  ...
  return (
    date.getFullYear() +
    "-" +
    lpadStr(date.getMonth() + 1) +
    "-" +
    lpadStr(date.getDate())
  );
}
...
function formatRetroPageContent(
  date: Date,
  scrummaster: string,
